...
class ParentCareApplication : Application() {
    ...
    private fun checkGoogleServicesFile(): Boolean {
        return try {
            ...
            options != null && options.projectId?.isNotEmpty() == true
        } catch (e: Exception) {
    ...
    private fun initializeFirestore() {
        try {
            ...
            // Configure Firestore settings using the new cache API to avoid deprecations
            val persistentCache = com.google.firebase.firestore.PersistentCacheSettings
                .newBuilder()
                .setSizeBytes(com.google.firebase.firestore.PersistentCacheSettings.CACHE_SIZE_UNLIMITED)
                .build()
    ...
    private fun validateSecurityConfiguration() {
        try {
            ...
            if (firebaseEnabled) {
                ...
                if (projectId != null && projectId.contains("dev")) {
                    issues.add("Development Firebase project detected")
                }
            }
    ...
    private fun validateApiKeyConfiguration() {
        ...
        Log.d(TAG, "🔑 API key configuration validated")
    }
