...
class ParentCareNavigationCoordinatorTest {
     ...
     */
    @Test
    fun `concurrent navigation events are handled sequentially`() = runTest {
        ...
        coordinator.navigationEvents.test {
            ...
            // Now attempt second navigation after first completes
            coordinator.onNavigationEvent(secondEvent)
            testDispatcher.scheduler.advanceUntilIdle()
            
            // Should receive second event
            val secondEmittedEvent = awaitItem()
            assertEquals(secondEvent, secondEmittedEvent)
        }
    }
